...
async def set_cmdm_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"🎬 [set_cmdm] Media check: photo={has_photo}, animation={has_anim}, video={has_video}")
    
    if not (has_photo or has_anim or has_video):
    ...
    if not context.args or len(context.args) < 1:
        ...
        await reply_and_delete(update, "❌ Використання: /set_cmdm <назва_команди>")
        return
    ...
    if db.add_personal_command_media(cmd_info['id'], media_type, file_id):
        logger.info(f"✅ [set_cmdm] Медіа успішно додано до команди '{cmd_name}'")
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and (update.message.photo or update.message.video or 
...
async def handle_text_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Перевіряємо персональні команди (тільки на reply)
    if update.message.reply_to_message:
        ...
        if cmd_info:
            ...
            # @s2 = одержувач (з клікабельним посиланням)
            target_user = update.message.reply_to_message.from_user
            target_id = target_user.id if target_user else user_id
            target_name = get_display_name(target_id, target_user.full_name if target_user else "Невідомий")
            ...
            result_text = cmd_info['template'].replace('@s1', clickable_s1).replace('@s2', clickable_s2).replace('@t', extra_text)
            
            # Отримуємо медіа, якщо є
            media = db.get_personal_command_media(cmd_info['id'])
            
            if media:
                try:
                    if media['type'] == 'photo':
                        await context.bot.send_photo(update.effective_chat.id, photo=media['file_id'], caption=result_text, parse_mode="HTML")
                    elif media['type'] == 'animation':
                        await context.bot.send_animation(update.effective_chat.id, animation=media['file_id'], caption=result_text, parse_mode="HTML")
                    elif media['type'] == 'video':
                        await context.bot.send_video(update.effective_chat.id, video=media['file_id'], caption=result_text, parse_mode="HTML")
                except Exception as e:
...
async def rezerv_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Генеруємо код резервної копії (чексума)
        backup_hash = hashlib.sha256(backup_json.encode()).hexdigest()[:12].upper()
...
📋 КОД РЕЗЕРВНОЇ КОПІЇ:
<code>{backup_hash}</code>

🔄 ДЛЯ ВІДНОВЛЕННЯ:
1️⃣ Надішліть цей код: <code>{backup_hash}</code>
2️⃣ Або скиньте QR-картинку
...
📦 Розмір: {len(backup_json)} байт"""
        ...
        await reply_and_delete(update, "✅ Резервну копію створено! Перевірте приватні повідомлення.", delay=60)
        
        # Зберігаємо бекап-код для імпорту (зберігаємо кешовано)
...
def setup_handlers(application):
    ...
    command_names = [
        ...
        "del_cmd", "doubler", "set_personal", "set_cmdm", "del_personal", "role_cmd", "admin_list", "rezerv",
    ]
