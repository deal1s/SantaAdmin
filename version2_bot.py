...
/custom_main - встановити кастомне ім'я адміну
...
/birthdays - показати список днів народження
/addb - додати день народження користувачу
/delb - видалити день народження за порядком (або свій без аргумента)
/profile - переглянути профіль користувача
...
async def allcmd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = """🌟 ВСІ ДОСТУПНІ КОМАНДИ ВЛАСНИКА (70 КОМАНД)
...
/custom_main - встановити кастомне ім'я адміну (reply)

🔧 УПРАВЛІННЯ ГНОМАМИ:
/add_gnome - додати гнома
/remove_gnome - видалити гнома

🚫 МОДЕРАЦІЯ (з часовим таймером!):
/ban_s - тихий бан користувача (reply)
/ban_t - публічний бан користувача (reply). Формат: /ban_t [причина]
/unban_s - тихе розблокування користувача
/unban_t - публічне розблокування користувача (reply)
/mute_s - тихий мут користувача (reply)
/mute_t - публічний мут (reply). Формат: /mute_t [час] [причина]
  ⏱️ Приклад: /mute_t 1m спам (мут на 1 хвилину, потім автоматичний розмут!)
  Час: 30s, 1m, 5m, 1h, 2h
/unmute_s - тихе розмут користувача (reply)
/unmute_t - публічне розмут (reply)
/kick - вигнати учасника з чату (reply). Формат: /kick @username [причина]
/nah - додати користувача в чорний список
...
/mymt - встановити опис профілю (до 300 символів)
/del_mymt - видалити опис профілю
/hto - переглянути інформацію про користувача (без аргумента - ваша)
/htop - призначити кастомну посаду користувачу (reply на message)
/del_htop - видалити кастомну посаду
/profile - переглянути профіль користувача

📝 НОТАТКИ ТА НАГАДУВАННЯ (доступно ДЛЯ ВСІХ):
/note - зберегти нотатку (Приклад: /note Привіт)
/notes - показати свої нотатки (можете переглядати чужі за ID)
/delnote - видалити нотатку за номером (Приклад: /delnote 1)
/reminder - створити нагадування для себе
/reminde - створити нагадування для іншого користувача
...
/birthdays - показати список днів народження
/addb - додати день народження користувачу
/delb - видалити день народження за порядком
...
🎯 ПЕРСОНАЛЬНІ КОМАНДИ:
/set_cmd - встановити текстовий дублер команди
/del_cmd - видалити дублер команди
/set_personal - створити персональну команду (плейсхолдери: @s1, @s2, @t)
/set_cmdm - додати медіа до персональної команди (reply на фото/гіф/відео)
/del_personal - видалити персональну команду
/role_cmd - показати список всіх персональних команд
...
ДЛЯ ВСІХ:
/help - команди для звичайних користувачів
/profile_set - всі команди налаштування профілю
/notes - список своїх нотаток

ДЛЯ ГНОМІВ І ВИЩЕ (Гноми, Head Admins, Власник):
/admin_list - список адміністраторів
/online_list - список адмінів онлайн (sayon/sayson)
/helpg - команди для гномів

ДЛЯ HEAD ADMINS І ВИЩЕ (Head Admins, Власник):
/helpm - команди для головних адмінів
/role_cmd - список персональних команд
/alarm - виклик адміністрації
/askgpt - запитання до GPT
...
⚠️ ПРИМІТКА: Старої команди /cmdall НЕ ІСНУЄ
✅ Використовуйте /allcmd для повного списку команд"""
...
async def giveperm_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"🔐 [giveperm_command] Роль користувача: {role}, is_owner: {is_owner(user_id)}")
    ...
    try:
        ...
        logger.info(f"🔐 [giveperm_command] Надання ВСІХ прав адміністратора...")
        await context.bot.promote_chat_member(
            chat_id=chat_id,
            user_id=target_user_id,
...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🎬 ОБРОБКА ФОТО З КОДОМ ДЛЯ ІМПОРТУ
    if update.message and update.message.photo:
        user_id = update.effective_user.id
        if is_owner(user_id):
            try:
                logger.info(f"📷 [import] Отримано фото від {user_id}")
                
                # 1️⃣ Спочатку перевіряємо опис картинки (caption)
                if update.message.caption:
                    logger.info(f"📝 [import] Картинка має опис, шукаємо код...")
                    caption_text = update.message.caption.upper()
                    
                    # Шукаємо 12-символьний код в описі
                    code_match = re.search(r'([A-F0-9]{12})', caption_text)
                    if code_match:
                        backup_code = code_match.group(1)
                        logger.info(f"✅ [import] Знайдено код в описі: {backup_code}")
                        await process_backup_import(update, context, backup_code)
                        return
                
                # 2️⃣ Якщо в описі коду немає, спробуємо розпізнати QR з самої картинки
                logger.info(f"📷 [import] Намагаємось розпізнати QR код з картинки...")
                file = await context.bot.get_file(update.message.photo[-1].file_id)
                file_path = f"/tmp/qr_{user_id}_{int(time_module.time())}.png"
                await file.download_to_drive(file_path)
                
                qr_code = await extract_qr_code(file_path)
                if qr_code:
                    logger.info(f"✅ [import] QR код розпізнано: {qr_code}")
                    await process_backup_import(update, context, qr_code)
                    try:
                        os.remove(file_path)
                    except:
                        pass
                ...
                # ❌ Ні коду в описі, ні QR не знайдено
                logger.warning(f"⚠️ [import] Не знайдено коду у фото")
                
                # 🗑️ ВИДАЛЯЄМО ОРИГІНАЛЬНЕ ФОТО
                try:
                    if update.message and update.message.message_id:
                        await context.bot.delete_message(
                            chat_id=update.effective_chat.id,
                        ...
                        logger.info(f"🗑️ [import] Фото видалено")
                except Exception as del_err:
                    logger.warning(f"⚠️ [import] Не вдалось видалити фото: {del_err}")
                
                # Надсилаємо помилку
                await reply_and_delete(
                    update,
                    "❌ Код не знайдено у фото!\n\n💡 Спробуй:\n1. Надішли картинку з оригінальним описом (містить код)\n2. АБО напиши код вручну (12 символів)\n3. АБО використай новий /rezerv",
                    delay=60
                )
                try:
                    os.remove(file_path)
                ...
                logger.warning(f"⚠️ [import] Помилка обробки фото: {e}")
                
                # 🗑️ Спробуємо видалити фото при помилці
                try:
                    if update.message and update.message.message_id:
                        await context.bot.delete_message(
                            chat_id=update.effective_chat.id,
                        ...
                        logger.info(f"🗑️ [import] Фото видалено (при помилці)")
                except:
                    pass
        ...
        logger.debug(f"📷 [handle_any_message] Ігнорування фото від {update.effective_user.id}")
        return
    
    # 🎬 Ігноруємо інші МЕДІА файли (гіф, відео, аудіо, стікери тощо)
    if update.message and (update.message.video or 
                           update.message.animation or update.message.document or 
                           update.message.audio or update.message.voice or 
                           update.message.sticker):
        logger.debug(f"📷 [handle_any_message] Ігнорування медіа-файлу від {update.effective_user.id}")
        return
...
async def handle_text_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"📝 [handle_text_commands] ВХІД в функцію! Update type: {type(update)}")
    ...
    logger.info(f"📝 [handle_text_commands] Нове текстове повідомлення від {user_id}: '{text}'")
    ...
    if not is_admin:
        logger.debug(f"📝 [handle_text_commands] Користувач {user_id} не адміністратор, ігноруємо")
        return
        ...
        if is_owner_user or source_chat_id == chat_id:
            ...
            try:
                if mode == "sayon":
                    ...
                    logger.info(f"📨 [handle_text_commands] Повідомлення успішно пересилано з підписом")
                    ...
                    logger.info(f"📨 [handle_text_commands] Повідомлення успішно пересилано анонімно")
            except Exception as e:
                logger.error(f"❌ Помилка автопересилання: {e}")
    ...
    logger.info(f"🔤 [handle_text_commands] Пошук алiаса для: '{first_word}' (chars: {[ord(c) for c in first_word[:3]]})")
    alias_cmd = db.get_command_alias(update.effective_chat.id, first_word)
    if alias_cmd:
        logger.info(f"✅ [handle_text_commands] Знайдено алiас '{first_word}' -> '{alias_cmd}' від {user_id}")
        ...
        if cmd in COMMAND_HANDLERS:
            logger.info(f"🔤 Виконую алiас команду '{cmd}' через COMMAND_HANDLERS з args: {context.args}")
            await COMMAND_HANDLERS[cmd](update, context)
        else:
            logger.warning(f"⚠️ Команда '{cmd}' не знайдена в COMMAND_HANDLERS")
        return
    else:
        logger.debug(f"❌ [handle_text_commands] Алiас '{first_word}' не знайдено в БД для чату {update.effective_chat.id}")
    
    # Перевіряємо персональні команди (працюють ЗІ reply АБО з @username)
    # Отримуємо всі персональні команди і шукаємо від найдовшої до найкоротшої
    ...
    for cmd in all_commands:
        ...
        if text.lower().startswith(cmd['name'].lower()):
            ...
            logger.info(f"🎭 Знайдена персональна команда '{cmd_name_used}' від {user_id}")
            break
    ...
    if cmd_info:
        ...
        # 🔄 НОВОЕ: Шукаємо @username в додатковому тексті
        ...
        if username_match:
            ...
            try:
                ...
                if db_user:
                    ...
                    logger.info(f"✅ Знайдено в БД: @{found_username} -> {target_name}")
            except Exception as e:
            ...
            if not target_id:
                try:
                    ...
                    if found_user:
                        ...
                        extra_text_for_output = extra_text.replace(f"@{found_username}", "").strip()
                        logger.info(f"✅ Знайдено в Telegram API: @{found_username} -> {target_name}")
                except Exception as e:
                    logger.warning(f"⚠️ Не вдалось знайти користувача @{found_username}: {e}")
        ...
        if target_id and clickable_s2:
            ...
            if media_list:
                ...
                logger.info(f"🎲 [personal_cmd] Медіа в списку: {[m['type'] for m in media_list]}")
                logger.info(f"🎲 [personal_cmd] Вибрано: #{selected_media['id']} - {selected_media['type']} (file_id: {selected_media['file_id'][:20]}...)")
...
def setup_handlers(application):
    ...
    command_names = [
        ...
        "sayb", "sayu", "alarm", "broadcast", "hto", "note", "notes", "delnote",
        "reminder", "reminde", "birthdays", "addb", "delb", "setbgif", "setbtext", "previewb",
        "adminchat", "userchat", "logchannel", "testchannel", "santas", "deltimer",
        "restart", "profile", "profile_set", "myname", "mym", "mymt", "del_myname", "del_mym",
        "del_mymt", "giveperm", "giveperm_simple", "removeperm", "custom_main", "set_cmd", 
        "del_cmd", "doubler", "set_personal", "set_cmdm", "del_personal", "set_adminm", "del_adminm", "role_cmd", "admin_list", "rezerv",
    ]
