...
async def allcmd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = """🌟 ВСІ ДОСТУПНІ КОМАНДИ ВЛАСНИКА (72 КОМАНДИ)
...
/notes - показати свої нотатки (можете переглядати чужі за ID)
  Приклад: /notes (ваші) або /notes @username (чужі)
/delnote - видалити нотатку за номером
  Приклад: /delnote 1
/reminder - створити нагадування для себе
/reminde - створити нагадування для іншого користувача
...
async def ban_t_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=f"🚫 Користувач {target_display} заблокований.\nПричина: {reason}",
            parse_mode=None
        )
...
async def unban_t_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        target_mention = f"<a href='tg://user?id={target_user['user_id']}'>{target_user['full_name']}</a>"
        await context.bot.send_message(
            chat_id=USER_CHAT_ID,
            text=f"✅ {target_mention} розблокований.",
            parse_mode="HTML"
        )
...
async def mute_t_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=USER_CHAT_ID,
            text=f"🔇 {target_mention} замучений.\nПричина: {reason}",
            parse_mode="HTML"
        )
...
async def unmute_t_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=USER_CHAT_ID,
            text=f"🔊 {target_mention} розмучений.",
            parse_mode="HTML"
        )
...
async def role_cmd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показати список всіх рольових команд"""
    save_user_from_update(update)
    if not update.effective_user or not update.effective_chat:
        return
...
async def admin_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_owner(user_id) and db.get_role(user_id) not in ["head_admin", "gnome"]:
        await reply_and_delete(update, "❌ Команда доступна тільки для адміністраторів!", delay=60)
        return
