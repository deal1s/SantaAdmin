...
async def handle_any_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and (update.message.video or 
                           ...
                           update.message.sticker or update.message.photo):
        return
...
async def handle_text_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"🎭 [personal_commands] Знайдено {len(all_commands) if all_commands else 0} персональних команд для чату {update.effective_chat.id}")
    if all_commands:
        logger.info(f"🎭 [personal_commands] Список: {[cmd['name'] for cmd in all_commands]}")
    ...
    if text.startswith("одружити"):
        logger.info(f"💍 [handle_text_commands] Текстова команда 'одружити' від {user_id}, роль: {role}")
        ...
        if "marry" in COMMAND_HANDLERS:
            ...
            if "marry_command" in globals():
                ...
                logger.warning(f"⚠️ Команда 'marry' не знайдена в COMMAND_HANDLERS або як функція")
                await reply_and_delete(update, "❌ Команда одружити не знайдена", delay=5)
        return
...
async def marry_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (is_owner(user_id) or is_head_admin(user_id)):
        await reply_and_delete(update, "❌ Тільки власник або головний адмін можуть одружувати!", delay=5)
        return
    
    # Extract mentions from arguments
    args = context.args if context.args else []
    if len(args) < 2:
        await reply_and_delete(update, "❌ Використання: /marry @user1 @user2", delay=5)
        return
    
    logger.info(f"💍 [marry] Спроба одружити {args[0]} з {args[1]} від {user_id}")
        ...
        try:
            ...
            if user_data:
                if arg_idx == 0:
                    ...
                    user1_name = get_display_name(user1_id, user_data.get('full_name', 'Невідомий'))
                else:
                    user2_id = user_data['user_id']
                    user2_name = get_display_name(user2_id, user_data.get('full_name', 'Невідомий'))
                logger.info(f"💍 Знайдено користувача {username} -> {user_data['user_id']}")
        except Exception as e:
            logger.warning(f"⚠️ Не вдалось знайти {username}: {e}")
    
    if not user1_id or not user2_id:
        await reply_and_delete(update, "❌ Не вдалось знайти користувачів. Використайте: /marry @user1 @user2", delay=5)
        return
    ...
    if spouse1 or spouse2:
        ...
        await reply_and_delete(update, f"❌ Один з користувачів вже одружений!", delay=5)
        return
    ...
    try:
        ...
        admin_name = get_display_name(user_id, update.effective_user.full_name or "Santa")
...
async def handle_member_left(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if spouse_id:
        ...
        left_user_name = get_display_name(left_user_id, left_user.full_name or "Невідомий")
        spouse_data = db.get_user(spouse_id)
        spouse_name = get_display_name(spouse_id, spouse_data.get('full_name', 'Невідомий') if spouse_data else 'Невідомий')
        
        logger.info(f"💔 [member_left] {left_user_name} ({left_user_id}) був одружений з {spouse_name} ({spouse_id})")
        ...
        try:
            ...
            logger.error(f"❌ Помилка при відправці повідомлення про розлучення: {e}")
...
def setup_handlers(application):
    ...
    command_names = [
        ...
        "del_cmd", "doubler", "set_personal", "set_cmdm", "del_personal", "set_adminm", "del_adminm", "role_cmd", "admin_list", "rezerv", "marry",
    ]
