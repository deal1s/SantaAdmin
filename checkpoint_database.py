...
class Database:
    ...
    def add_personal_command_media(self, command_id: int, media_type: str, file_id: str) -> bool:
        """Додати медіа до персональної команди"""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            # Видаляємо старе медіа для цієї команди
            cursor.execute('DELETE FROM personal_command_media WHERE command_id = ?', (command_id,))
            # Додаємо нове
            cursor.execute('''
    ...
    def get_personal_command_media(self, command_id: int) -> Optional[Dict]:
        """Отримати медіа персональної команди"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            SELECT media_type, file_id FROM personal_command_media WHERE command_id = ? LIMIT 1
        ''', (command_id,))
        result = cursor.fetchone()
        conn.close()
        return {"type": result[0], "file_id": result[1]} if result else None
    ...
    def import_all_backup(self, backup_data: Dict[str, Any]) -> bool:
